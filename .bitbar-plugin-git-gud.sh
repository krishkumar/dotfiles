...
COUNT=`cd $MASTER;/usr/local/bin/git-standup | grep "<krishna>" | wc -l`
if [ $COUNT = 0 ]
then
    COUNT=""
...
cd $MASTER
#/usr/local/bin/git-standup | sed 's/\/Users\/krishna\/master\///;n'
/usr/local/bin/git-standup | sed "s@$MASTER@$INDICATOR@;n"
