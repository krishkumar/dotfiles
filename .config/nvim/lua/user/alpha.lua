...
local buttons = {
  ...
  val = {
    ...
    startify.button(
      'd',
      '  Dotfiles',
      "<cmd>lua require('telescope.builtin').find_files({ search_dirs = { os.getenv('HOME') .. '/master/dotfiles' }, hidden=true })<CR>"
    ),
    startify.button('u', '  Update plugins', ':PackerSync<CR>'),
    startify.button('c', '  Neovim', ':cd ~/.config/nvim/<CR> :e init.lua<CR>'),
    startify.button('k', '  Kitty', ':e ~/.config/kitty/kitty.conf<CR>'),
